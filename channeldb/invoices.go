...
func getPaymentHash(i *Invoice) ([32]byte, error) {
	...
	if i.Terms.ExternalPreimage {
		...
		// For local preimages, we calculate the hash ourselves. This also ensures
		// that an invoice an identical payment hash doesn't already exist within
		// the index.
...
func (d *DB) AddInvoice(newInvoice *Invoice) (uint64, error) {
	...
	err := d.Update(func(tx *bolt.Tx) error {
		...
		// Grabs a payment hash, either from a local source OR from an external
		// node in the case of an External Preimage
